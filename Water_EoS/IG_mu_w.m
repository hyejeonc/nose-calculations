...
% Derivatives
mu.dT = -1*S0+ktp2.R.*(log(P./ktp2.Pref)+term);  % Temperature [J/kmole K]
mu.dP = T*ktp2.R./P;                            % Pressure [J/kmole P]
...
for i=1:ktp2.nc
    for j=1:ktp2.nc
        ...
        if i==j
        mu.dZ(i,j) = ktp2.R.*T.*(1./Z(i)).*(1-Z(i));
        mu.dy(i,j) = mu.dZ(i,j) ./ (1-Z(i));
        else
        mu.dZ(i,j) = ktp2.R.*T.*(1./Z(i)).*(-Z(i));
        mu.dy(i,j) = mu.dZ(i,j) ./ (1-Z(i));
        end
