...
function [Zfac_struct, Temp]=TP_Zfac(T,P,Z,phase)
...
global ktp2
    ...
    % Scaled parameter A [-]
    A_s=Temp.a*P/((T*ktp2.R)^2);
    
    % Scaled parameter B [-]
    B_s=Temp.b*P/(T*ktp2.R);
    
    % Parameters
    m1_m2_p=(ktp2.EoS.m1+ktp2.EoS.m2);
    m1_m2_m=(ktp2.EoS.m1*ktp2.EoS.m2);
    ...
    if length(r_real)>1  % More than one root
        ...
        if phase==0    % Minimum Gibbs energy
            ...
            G_res_g=CB_Zfac_Gres_w(T,P,Zfac_g,Temp);
            G_res_l=CB_Zfac_Gres_w(T,P,Zfac_l,Temp);
    ...
    Zfac_dT=P*dVdT/(ktp2.R*T)-Zfac/T;      % (dZfac/dT)_{p,Ni}
    
    % Pressure derivative of the compressibility
    Zfac_dP=P/(Temp.PV*ktp2.R*T) + Zfac/P;   % (dZfac/dP)_{T,Ni}
    
    % Composition derivative of the compressibility
    DPDNI=Temp.PN+Temp.PA*Temp.AI+...     % N_tot*(dP/dNi)
        Temp.PB*Temp.BI;
    DVDNI=-DPDNI/Temp.PV;                 % (dV/dNi)
    Zfac_dN=P/(ktp2.R*T)*DVDNI - Zfac;     % Scaled compressibility derivative
    % (1/N_tot)*(dZfac/dNi)
    
    % Allocate values to struct
    Zfac_struct.Zfac=Zfac;
    Zfac_struct.c=P/(Zfac*ktp2.R*T);       % Concentration [mol/m^3]
